...
describe('jobProfileService', () => {
  ...
  const API_BASE_URL = import.meta.env.VITE_PREPROD_URL || 'http://localhost:3000/api';
...
expect(mockFetch).toHaveBeenNthCalledWith(2, `${API_BASE_URL}/client/all`, {
  ...
  describe('createJobProfile', () => {
    const validJobProfileData: Omit<JobProfilePayload, 'jobProfileId'> = {
      clientId: 1,
      departmentId: 101,
  ...
  describe('updateJobProfile', () => {
    const updateData: Partial<Omit<JobProfilePayload, 'jobProfileId'>> = {
  ...
  describe('validateJobProfileRequest', () => {
    const validData: Partial<JobProfilePayload> = {
      clientId: 1,
      departmentId: 101,
