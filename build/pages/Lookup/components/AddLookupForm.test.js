...
describe("AddLookupForm Component", () => {
    ...
    const setup = (props = {}) => render(_jsx(AddLookupForm, { visible: true, onHide: onHide, onSuccess: onSuccess, ...props }));
    // --- 1️⃣ RENDER TEST ---
    ...
    it("submits form successfully and calls onSuccess + onHide", async () => {
        lookupService.create.mockResolvedValue({
            success: true,
    ...
    it("handles network error gracefully", async () => {
        lookupService.create.mockRejectedValue(new Error("Network error"));
        setup();
        fireEvent.change(screen.getByLabelText(/Tag/i), {
            target: { value: "net" },
        });
