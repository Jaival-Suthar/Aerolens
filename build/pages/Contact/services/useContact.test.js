...
describe('useContact', () => {
    ...
    describe('getClientDetails', () => {
        ...
        it('fetches client details successfully with access token', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith(`${mockApiUrl}/client/${mockClientId}`, {
                method: 'GET',
                headers: {
        ...
        it('throws error when clientId is missing', async () => {
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.getClientDetails(mockAccessToken, 0)).rejects.toThrow(/Client ID is required/);
            });
            expect(result.current.loading).toBe(false);
        ...
        it('throws error when response is not JSON', async () => {
            ...
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.getClientDetails(mockAccessToken, mockClientId)).rejects.toThrow(/Expected JSON but got text\/html/);
            });
            expect(result.current.loading).toBe(false);
        ...
        it('throws error when content-type header is missing', async () => {
            ...
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.getClientDetails(mockAccessToken, mockClientId)).rejects.toThrow(/Expected JSON but got unknown content type/);
            });
            expect(result.current.loading).toBe(false);
        ...
        it('handles API error response', async () => {
            ...
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.getClientDetails(mockAccessToken, mockClientId)).rejects.toThrow(/Client not found/);
            });
            expect(result.current.loading).toBe(false);
        ...
        it('handles network error', async () => {
            ...
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.getClientDetails(mockAccessToken, mockClientId)).rejects.toThrow(/Network error/);
            });
            expect(result.current.loading).toBe(false);
        ...
        it('handles HTTP error without custom message', async () => {
            ...
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.getClientDetails(mockAccessToken, mockClientId)).rejects.toThrow(/HTTP error! status: 500/);
            });
            expect(result.current.loading).toBe(false);
    ...
    describe('createContact', () => {
        ...
        it('creates contact successfully with access token', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith(`${mockApiUrl}/contact`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${mockAccessToken}`
        ...
        it('handles create contact API error', async () => {
            ...
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.createContact(mockAccessToken, mockContactData)).rejects.toThrow(/Invalid contact data/);
            });
            expect(result.current.loading).toBe(false);
        ...
        it('handles unknown error during contact creation', async () => {
            ...
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.createContact(mockAccessToken, mockContactData)).rejects.toBe('Unknown error object');
            });
            expect(result.current.loading).toBe(false);
    ...
    describe('updateContact', () => {
        ...
        it('updates contact successfully with all fields and access token', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith(`${mockApiUrl}/contact/1`, expect.objectContaining({
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${mockAccessToken}`
        ...
        it('throws error when contactId is missing', async () => {
            ...
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.updateContact(mockAccessToken, dataWithoutId)).rejects.toThrow(/Contact ID is required for update operation/);
            });
            expect(result.current.loading).toBe(false);
        ...
        it('handles update contact API error', async () => {
            ...
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.updateContact(mockAccessToken, mockUpdateData)).rejects.toThrow(/Contact not found/);
            });
            expect(result.current.loading).toBe(false);
    ...
    describe('deleteContact', () => {
        ...
        it('deletes contact successfully with access token', async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith(`${mockApiUrl}/contact/${mockContactId}`, expect.objectContaining({
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${mockAccessToken}`
        ...
        it('throws error when contactId is invalid', async () => {
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.deleteContact(mockAccessToken, 0)).rejects.toThrow(/Contact ID is required for delete operation/);
            });
            expect(result.current.loading).toBe(false);
            expect(result.current.error).toContain('Contact ID is required');
        });
        it('handles delete contact API error', async () => {
            ...
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.deleteContact(mockAccessToken, mockContactId)).rejects.toThrow(/Internal server error/);
            });
            expect(result.current.loading).toBe(false);
        ...
        it('handles HTTP error without message', async () => {
            ...
            const { result } = renderHook(() => useContact());
            await act(async () => {
                await expect(result.current.deleteContact(mockAccessToken, mockContactId)).rejects.toThrow(/HTTP error! status: 500/);
            });
            expect(result.current.loading).toBe(false);
            expect(result.current.error).toContain('HTTP error! status: 500');
        });
