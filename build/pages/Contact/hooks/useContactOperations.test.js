...
// --- Test Data ---
const MOCK_CLIENT = { clientId: 1, clientName: 'Mock Client' };
...
describe('useContactOperations', () => {
    ...
    describe('handleSaveContact - ADD Mode', () => {
        it('successfully calls createContact, shows success, and refreshes', async () => {
            const { result } = setupHook();
            const response = await act(() => result.current.handleSaveContact(MOCK_CONTACT_PAYLOAD, 'add', MOCK_CLIENT));
            expect(response.success).toBe(true);
            expect(mockCreateContact).toHaveBeenCalledWith('mock-token-123', // ✅ Add token as first param
            {
        ...
        it('shows error if client ID is missing in ADD mode', async () => {
            const { result } = setupHook();
            const response = await act(() => result.current.handleSaveContact(MOCK_CONTACT_PAYLOAD, 'add', null));
            expect(response.success).toBe(false);
    ...
    describe('handleSaveContact - EDIT Mode', () => {
        ...
        it('successfully calls updateContact, shows success, and refreshes', async () => {
            const { result } = setupHook();
            const response = await act(() => result.current.handleSaveContact(UPDATE_PAYLOAD, 'edit', MOCK_CLIENT));
            expect(response.success).toBe(true);
            expect(mockUpdateContact).toHaveBeenCalledWith('mock-token-123', // ✅ Add token as first param
            {
                clientContactId: MOCK_EDIT_CONTACT.clientContactId,
        ...
        it('handles backwards compatibility using contactId fallback', async () => {
            ...
            await act(() => result.current.handleSaveContact(payloadWithContactId, 'edit', MOCK_CLIENT));
            expect(mockUpdateContact).toHaveBeenCalledWith('mock-token-123', // ✅ Add token as first param
        ...
        it('shows error if Contact ID is missing in EDIT mode', async () => {
            ...
            const response = await act(() => result.current.handleSaveContact(invalidPayload, 'edit', MOCK_CLIENT));
            expect(response.success).toBe(false);
    ...
    describe('handleDeleteContact', () => {
        it('successfully calls deleteContact, shows success, and refreshes', async () => {
            const { result } = setupHook();
            const response = await act(() => result.current.handleDeleteContact(MOCK_EDIT_CONTACT));
            expect(response.success).toBe(true);
            expect(mockDeleteContact).toHaveBeenCalledWith('mock-token-123', // ✅ Add token as first param
        ...
        it('shows error if Contact ID is missing in DELETE operation', async () => {
            ...
            const response = await act(() => result.current.handleDeleteContact(contactWithoutId));
            expect(response.success).toBe(false);
    ...
    describe('validateContactSelection', () => {
        ...
        it('returns false and shows error when clientContactId is missing', () => {
            const { result } = setupHook();
            const invalidContact = { clientId: 1, contactPersonName: 'Invalid' };
