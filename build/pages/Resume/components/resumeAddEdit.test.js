...
import { updateCandidate, uploadResume, } from "../services/useResume";
...
vi.mock("primereact/dialog", () => ({
    Dialog: ({ visible, header, children, footer }) => visible ? (_jsxs("div", { "data-testid": "dialog", children: [_jsx("h2", { children: header }), children, _jsx("div", { "data-testid": "dialog-footer", children: footer })] })) : null,
}));
vi.mock("primereact/inputtext", () => ({
    InputText: (props) => (_jsx("input", { id: props.id, value: props.value, onChange: props.onChange, onBlur: props.onBlur, className: props.className, placeholder: props.placeholder, "data-testid": props.id })),
}));
vi.mock("primereact/dropdown", () => ({
    Dropdown: (props) => (_jsxs("select", { id: props.id, value: props.value, onChange: (e) => props.onChange({ value: e.target.value }), onBlur: props.onBlur, className: props.className, "data-testid": props.id, children: [_jsx("option", { value: "", children: "Select" }), props.options.map((opt) => (_jsx("option", { value: opt.value, children: opt.label }, opt.value)))] })),
}));
vi.mock("primereact/inputnumber", () => ({
    InputNumber: (props) => (_jsx("input", { id: props.id, type: "number", value: props.value, onChange: (e) => props.onValueChange({ value: Number(e.target.value) }), onBlur: props.onBlur, className: props.className, "data-testid": props.id })),
}));
vi.mock("primereact/fileupload", () => ({
    FileUpload: (props) => (_jsx("input", { "data-testid": "file-upload", type: "file", onChange: (e) => props.onSelect({ files: Array.from(e.target.files ?? []) }), className: props.className })),
}));
vi.mock("../../../shared/DialogAddEditButton", () => ({
    __esModule: true,
    default: ({ label, onClick, severity }) => (_jsx("button", { onClick: onClick, className: severity, children: label })),
}));
...
vi.mock("react-icons/fa", () => ({
    FaCheck: () => _jsx("span", { "data-testid": "icon-check", children: "\u2713" }),
}));
// ---------- HELPERS ----------
const renderComponent = (props = {}) => {
    ...
    return render(_jsx(ResumeAddEdit, { ...defaultProps, ...props }));
};
// ---------- TESTS ----------
describe("ResumeAddEdit Component", () => {
    ...
    it("calls createCandidate when adding a new candidate", async () => {
        ...
        const getInput = (label) => screen.getByText(label).parentElement?.querySelector("input");
        const getSelect = (label) => screen.getByText(label).parentElement?.querySelector("select");
