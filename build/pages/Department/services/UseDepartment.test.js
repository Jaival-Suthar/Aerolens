...
import { getDepartments, addDepartment, updateDepartment, deleteDepartment, } from '../../Department/services/useDepartment';
...
describe('departmentService', () => {
    ...
    describe('getDepartments', () => {
        it('fetches and returns departments on success', async () => {
            ...
            expect(fetch).toHaveBeenCalledWith(`${API_BASE_URL}/client/1`, expect.objectContaining({
                credentials: 'include',
                method: 'GET',
            }));
    ...
    describe('addDepartment', () => {
        ...
        it('throws error with API error message', async () => {
            const errorResponse = { success: false, message: 'Duplicate department' };
            vi.mocked(fetch).mockResolvedValueOnce({
                ok: false,
                status: 400,
    ...
    describe('updateDepartment', () => {
        ...
        it('throws error if neither name nor description provided', async () => {
            await expect(updateDepartment('mock-token-123', { departmentId: 5 })).rejects.toThrow('At least one field required for update' // Match the actual error message
            );
    ...
    describe('deleteDepartment', () => {
        ...
        it('throws error with API error message', async () => {
            const errorResponse = { success: false, message: 'Delete failed' };
            vi.mocked(fetch).mockResolvedValueOnce({
                ok: false,
                status: 400,
                text: async () => JSON.stringify(errorResponse),
