...
describe('AuthProvider', () => {
    ...
    beforeEach(() => {
        ...
        profileStore.useProfileStore.mockReturnValue({
    ...
    describe('Initialization', () => {
        ...
        it('fetches profile on mount when token exists', async () => {
            ...
            await waitFor(() => {
                expect(fetchMock).toHaveBeenCalledWith(`${API_BASE}/auth/profile`, expect.objectContaining({
        ...
        it('refreshes token when profile fetch returns 401', async () => {
            ...
            renderHook(() => useAuth(), { wrapper });
            await waitFor(() => {
                expect(fetchMock).toHaveBeenCalledWith(`${API_BASE}/auth/refresh`, expect.objectContaining({
                    method: 'POST',
    ...
    describe('login', () => {
        it('successfully logs in and sets token and profile', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(`${API_BASE}/auth/login`, expect.objectContaining({
                method: 'POST',
        ...
        it('throws error when login fails', async () => {
            ...
            const { result } = renderHook(() => useAuth(), { wrapper });
            await expect(result.current.login('user@example.com', 'wrong-password')).rejects.toThrow(errorMessage);
        });
    });
    describe('logout', () => {
        it('clears token and profile on logout', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(`${API_BASE}/auth/logout`, expect.objectContaining({
                method: 'POST',
                credentials: 'include',
            }));
            expect(result.current.accessToken).toBeNull();
    ...
    describe('logoutAll', () => {
        it('logs out from all devices when authenticated', async () => {
            ...
            expect(fetchMock).toHaveBeenCalledWith(`${API_BASE}/auth/logout-all`, expect.objectContaining({
                method: 'POST',
                headers: { Authorization: `Bearer ${mockToken}` },
                credentials: 'include',
            }));
